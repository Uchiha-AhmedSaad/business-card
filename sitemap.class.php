	...
	public function generate_sitemap(){
      ...
      xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9
        ...
        foreach($this->sitemap_urls as $url) 
		{
			...
			$url_tag->addChild("changefreq", 'daily');
			$url_tag->addChild("priority", '0.5');
			$url_tag->addChild("lastmod", date("Y-m-d"));
			
		}
		return $sitemap->asXML();
	}
// updated by mohamed hassan batrahost.com 3-11-2013
...
$sitemapheader = '<?xml version="1.0" encoding="UTF-8"?>
	...
	asort($this->sitemap_urls);
	$a = 0;
	foreach ($this->sitemap_urls as $i => $x){
if(stristr($x,"./")==false){
	$a++;
	$sitemaplinks = '<url>
	<loc>'.$x.'</loc>
